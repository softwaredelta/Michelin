...
module.exports = class Metric {
    ...
    static async averageGradeCur (fastify, dFilter, zone, user) {
      const connection = await fastify.mysql.getConnection()
      let queryString = "SELECT AVG(f.exterior_grade) AS 'EXTERIOR', AVG(f.interior_grade) AS 'INTERIOR', AVG(f.client_grade) AS 'CLIENT', AVG(f.store_manager_grade) AS 'MANAGER' FROM form f, sellingpoint sp WHERE f.date >= DATE_ADD(CURRENT_DATE(),INTERVAL -6 MONTH) AND f.sp_name = sp.name "+ dFilter + zone + user
      const rows = await connection.query(
        queryString
      )
      connection.release()
    ...
    static async formsByMonthManager (fastify, zone, user) {
      const connection = await fastify.mysql.getConnection()
      let queryString = "SELECT MONTH(f.date) as 'MONTH', COUNT(f.id_form) as 'COUNT' FROM form f, sellingpoint sp WHERE f.date >= DATE_ADD(CURRENT_DATE(),INTERVAL -6 MONTH) AND ((f.id_user IN (SELECT u.id_user FROM users u WHERE u.id_manager = " + user + ")) OR f.id_user = " + user + ") AND f.sp_name = sp.name " + zone + " GROUP BY MONTH(f.date)"
      const rows = await connection.query(
        queryString
      )
      connection.release()
    ...
    static async formsByMonthAdmin (fastify, zone) {
      const connection = await fastify.mysql.getConnection()
      let queryString = "SELECT MONTH(f.date) as 'MONTH', COUNT(f.id_form) as 'COUNT' FROM form f, sellingpoint sp WHERE f.date >= DATE_ADD(CURRENT_DATE(),INTERVAL -6 MONTH) AND f.sp_name = sp.name " + zone + " GROUP BY MONTH(f.date)"
      const rows = await connection.query(
        queryString
      )
      connection.release()
    ...
    static async averageTimeByMonth (fastify, zone, user) {
      const connection = await fastify.mysql.getConnection()
      let queryString = "SELECT MONTH(f.date) as 'MONTH', AVG(f.duration) as 'DURATION' FROM form f, sellingpoint sp WHERE f.date >= DATE_ADD(CURRENT_DATE(),INTERVAL -6 MONTH) AND f.sp_name = sp.name " + zone + user + " GROUP BY MONTH(f.date)"
      const rows = await connection.query(
        queryString
      )
      connection.release()
